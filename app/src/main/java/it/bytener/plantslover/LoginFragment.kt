...
class LoginFragment : Fragment(R.layout.fragment_login) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.viewRegister.setOnClickListener {
            ...
            animOut(binding.leafBottom,
                ...
                val extras = FragmentNavigatorExtras(binding.constraintlayoutUserData to "user_data")
    ...
    private fun fadeOutTitle() {
        val fadeOutAnimator = ObjectAnimator.ofFloat(binding.textViewLogin, View.ALPHA, 1f, 0f).setDuration(400)
        fadeOutAnimator.start()
    }

    private fun translateRegisterButton() {
        val translateViewAnimator = ObjectAnimator.ofFloat(binding.viewRegister, View.TRANSLATION_X, 0f, -binding.viewRegister.layoutParams.width * 1.0f).setDuration(400)
        translateViewAnimator.start()
        val translateTextAnimator = ObjectAnimator.ofFloat(binding.textViewRegister, View.TRANSLATION_X, 0f, -binding.viewRegister.layoutParams.width * 1.0f).setDuration(400)
    ...
    private fun animOut(view: View, startPosition: Float, endPosition: Float, callback: () -> Unit) {
        ...
        fadeOutTitle()
        translateRegisterButton()
    }

    private fun fadeInTitle() {
        val fadeOutAnimator = ObjectAnimator.ofFloat(binding.textViewLogin, View.ALPHA, 0f, 1f).setDuration(500)
        fadeOutAnimator.start()
    }
}